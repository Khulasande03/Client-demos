 ...
 *
 * Sources hero components from the open-source serafimcloud/21st GitHub repo
 * (21st.dev blocks all bots/scrapers, but their code is MIT-licensed on GitHub).
 *
 * Pipeline:
 *   1. Clone/pull serafimcloud/21st (sparse, components only)
 *   2. Find all hero .tsx files
 *   3. Claude API → inject Framer Motion animations
 *   4. Write animated .tsx to ./components/heroes/
 *
 * Usage:
 *   npm run scrape              — scrape all heroes from repo
 *   npm run scrape -- --file path/to/hero.tsx  — animate single local file
 */
...
async function ensureRepo(): Promise<void> {
  try {
    ...
    console.log('📦 Cloning 21st.dev repo (components only)...');
    execSync(
      `git clone --depth=1 --filter=blob:none ${REPO_URL} ${REPO_DIR}`,
      { stdio: 'pipe' }
    );
  }
}
...
// ── Step 3: inject Framer Motion via Claude API ──────────────────────────────
async function injectFramerMotion(source: string, componentName: string): Promise<string> {
  ...
  const msg = await anthropic.messages.create({
    ...
    max_tokens: 4096,
    system: `You are an expert React/Framer Motion developer. Add Framer Motion animations to hero components.
...
- List items: use staggerChildren:0.08 on parent motion.div + each child has initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}}
- Hero background/image: add useScroll + useTransform for parallax, const {scrollY}=useScroll(); const bgY=useTransform(scrollY,[0,500],[0,-120]); apply style={{y:bgY}}
- Accessibility: const prefersReduced=useReducedMotion(); wrap all transition objects: transition={prefersReduced?{duration:0}:{...actual}}
...
- Return ONLY complete .tsx code, no markdown fences, no explanation`,
    messages: [{
      role: 'user',
      content: `Add Framer Motion animations to this hero component:\n\n${source}`,
    }],
  });

  const text = msg.content[0].type === 'text' ? msg.content[0].text : source;
...
async function main() {
  ...
  // Mode: single file passed via --file flag
  ...
  if (fileFlag !== -1 && process.argv[fileFlag + 1]) {
    ...
    console.log(`Found ${heroFiles.length} hero file(s) in repo\n`);
  }
  ...
  for (const heroPath of heroFiles) {
    ...
    try {
      ...
      await new Promise(r => setTimeout(r, 500)); // rate limit cushion
    } catch (err: any) {
  ...
  console.log(`\n${ok} new hero(es) animated → ${path.resolve(OUT_DIR)}`);
}
